...
agent-browser stream enable --sink wss://receiver.example.com/session/abc  # Push frames to an outbound receiver
agent-browser stream disable           # Stop streaming for the session
```

`stream status` returns the enabled state, active port, browser connection state, outbound sink state, and whether screencasting is active. `stream disable` tears the server down and removes the session's `.stream` metadata file.
...
```json
{
  ...
  "connected": true,
  "screencasting": true,
  "sinkUrl": "wss://receiver.example.com/session/abc",
  "sinkConnected": true
}
```
...
Use `stream enable --sink <ws-url>` to push the same sanitized status and frame messages to an outbound WebSocket receiver. This is intended for sandboxed environments where the browser can connect out to a trusted runtime channel, but the local stream server should not be exposed.

The outbound sink contributes stream demand, so frames start flowing even when no local dashboard WebSocket client is connected. If the sink falls behind, it drops stale broadcast messages instead of blocking browser automation or video recording.

## Relationship to screencast commands

`stream enable` creates the WebSocket server and keeps it available for the session. WebSocket clients and outbound sinks then trigger live frame delivery automatically.
