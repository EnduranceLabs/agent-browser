...
agent-browser stream enable --sink wss://receiver.example.com/session/abc  # Push frames to an outbound WebSocket receiver
...
Streaming is enabled automatically for all sessions. Use these commands to check status, re-enable on a specific port, push frames to an outbound receiver, or disable streaming.
